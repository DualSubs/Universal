		...
		case "application/json": {
			...
			switch (PLATFORM) {
				...
				case "Spotify": {
					...
					const translation = await Translator(Settings.Vendor, Settings.Method, fullText, Languages, Settings?.[Settings?.Vendor], Settings?.Times, Settings?.Interval, Settings?.Exponential);
					Console.debug(`$request.headers["app-platform"]: ${$request?.headers?.["app-platform"]}`);
				...
				case "application/vnd.google.protobuf":
					switch (PLATFORM) {
						case "YouTube": {
							...
							//Console.debug(`body: ${JSON.stringify(body)}`);
							Console.debug(`contents: ${JSON.stringify(body?.contents)}`);
							Console.debug(`continuationContents: ${JSON.stringify(body?.continuationContents)}`);
