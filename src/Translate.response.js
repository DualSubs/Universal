...
const $ = new ENVs("🍿️ DualSubs: 🔣 Universal v1.2.7(1) Translate.response");
const URI = new URIs();
const XML = new XMLs();
...
async function Translator(vendor = "Google", method = "Part", text = [], [source = "AUTO", target = "ZH"], API = {}, times = 3, interval = 100, exponential = true) {
	...
	switch (method) {
		...
		case "Part": // Part 逐段翻译
			...
			Translation = await Promise.all(parts.map(async part => await retry(() => new Translate($)[vendor](part, source, target, API), times, interval, exponential))).then(part => part.flat(Infinity));
			break;
		case "Row": // Row 逐行翻译
			Translation = await Promise.all(text.map(async row => await retry(() => new Translate($)[vendor](row, source, target, API), times, interval, exponential)));
			break;
	};
