...
const DataBase = {
	...
	Settings: {"Verify":{"GoogleCloud":{"Method":"Part","Mode":"Key","Auth":null},"Azure":{"Method":"Part","Version":"Azure","Region":null,"Mode":"Key","Auth":null},"DeepL":{"Method":"Part","Version":"Free","Auth":null}},"Advanced":{"Translator":{"Times":3,"Interval":100,"Exponential":true}},"Default":{"Switch":true,"Types":["Official","Google"],"Type":"Google","Languages":["ZH","EN"],"Language":"ZH","External":{"URL":null,"Offset":0,"ShowOnly":false},"Position":"Forward","CacheSize":6,"Tolerance":1000}},
...
!(async () => {
	...
	if (Settings.Switch) {
		// 找缓存
		const Indices = await getCache(Platform, Type, Settings, Caches);
		...
		for await (var language of Settings.Languages) {
			Cache[language] = await getMEDIA(Platform, PlayList, (Platform == "Netflix") ? "subtitles" : "SUBTITLES", language);
...
async function getCache(platform, type, settings, caches = {}) {
	$.log(`⚠ ${$.name}, Get Cache`, "");
	let Indices = {};
	Indices.Index = await getIndex(platform, settings, caches);
	if (Indices.Index !== -1) {
...
async function getMEDIA(platform = "", json = {}, type = "", langCode = "") {
	...
	async function setMEDIA(platform = "", data = {}, langCode = "") {
