...
const $ = new ENV("🍿️ DualSubs: 🔣 Universal v1.5.1(6) External.Lyrics.response.beta");
const URI = new URI$1();
const LRC = new LRCs();
...
async function searchTrack(vendor = "QQMusic", keyword = "", UAPool = []){
	...
	switch (vendor) {
		case "NeteaseMusicNodeJS": {
			...
			await $.http.get(searchRequest).then(response => {
		...
		default: {
			...
			await $.http.post(searchRequest).then(response => {
