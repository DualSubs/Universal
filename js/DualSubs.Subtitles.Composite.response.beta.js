...
const $ = new Env("🍿️ DualSubs: 🎦 Universal v0.8.10(4) Subtitles.Composite.response.beta");
const URL = new URLs();
const XML = new XMLs();
...
function WebVTT(opts) {
	return new (class {
		...
		parse(vtt = this.vtt) {
			const body_CUE_Regex = (this.opts.includes("milliseconds")) ? /^((?<index>\d+)[^])?(?<timeLine>(?<startTime>[0-9:.,]+) --> (?<endTime>[0-9:.,]+)) ?(?<options>.+)?[^](?<text>[\s\S]*)?$/
				: /^((?<index>\d+)[^])?(?<timeLine>(?<startTime>[0-9:]+)[0-9.,]+ --> (?<endTime>[0-9:]+)[0-9.,]+) ?(?<options>.+)?[^](?<text>[\s\S]*)?$/
