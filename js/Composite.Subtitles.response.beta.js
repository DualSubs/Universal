			...
			default:
				...
				while (index1 < length1 && index2 < length2) {
					...
					if (Math.abs(timeStamp1 - timeStamp2) <= Tolerance) {
						...
						else { index1++; index2++; }					}				}		}		//log(`✅ Composite JSON Subtitles, DualSub内容: ${JSON.stringify(DualSub)}`, "");
	...
	webVTT(Sub1 = {}, Sub2 = {}, Offset = this.Offset, Tolerance = this.Tolerance, Position = this.Position) {
		...
		while (index1 < length1 && index2 < length2) {
			...
			if (Math.abs(timeStamp1Next - timeStamp2Next) <= Tolerance) { index1++; index2++; }
				...
				else { index1++; index2++; }			}		}		//log(`✅ Composite webVTT Subtitles, DualSub内容: ${JSON.stringify(DualSub)}`, "");
	...
	spotifyLyric(Lyric1 = [], Lyric2 = [], Offset = this.Offset, Tolerance = this.Tolerance, Position = this.Position) {
		...
		while (index1 < length1 && index2 < length2) {
			...
			if (Math.abs(timeStamp1Next - timeStamp2Next) <= Tolerance) { index1++; index2++; }
				...
				else { index1++; index2++; }			}		}		//log(`✅ Composite Spotify Lyrics, Lyric: ${JSON.stringify(Lyric)}`, "");
